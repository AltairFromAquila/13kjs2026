...
export const waterGenerate = (waterColor: Color, botWaterColor: Color, foamColor: Color) => {
  for (let y = 0; y < kWaterWidth; ++y) {
    for (let x = 0; x < kWaterWidth; ++x) {
      ...
      const color = waterSample(u, v, waterColor, botWaterColor, foamColor);
