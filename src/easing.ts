...
import { kMathPi, mathCos, mathPow, mathSin } from "./math";
...
export const easeInOutQuad = (x: number): number => x < 0.5 ? 2 * x * x : 1 - mathPow(-2 * x + 2, 2) / 2;

export const easeInCubic = (x: number): number => x * x * x;

export const easeOutCubic = (x: number): number => 1 - mathPow(1 - x, 3);

export const easeInOutCubic = (x: number): number => x < 0.5 ? 4 * x * x * x : 1 - mathPow(-2 * x + 2, 3) / 2;
